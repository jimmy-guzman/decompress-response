...
import decompressResponse from '../dist/index.mjs'
...
test.before('setup', async () => {
  ...
  server.on('/non-compressed', (request, response) => {
    response.statusCode = 200
    response.setHeader('content-type', 'text/plain')
...
test('decompress raw-deflated content', async (t) => {
  const response = decompressResponse(
    await httpGetP(`${server.url}/deflateRaw`)
  )
...
test('does not ignore missing data', async (t) => {
  const response = decompressResponse(
    await httpGetP(`${server.url}/missing-data`)
  )
...
test('passthrough on non-compressed data', async (t) => {
  const response = decompressResponse(
    await httpGetP(`${server.url}/non-compressed`)
  )
